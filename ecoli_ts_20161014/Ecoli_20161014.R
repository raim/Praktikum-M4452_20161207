...
library("grofit")
source("~/programs/platexpress/R/interfaces.R") # hack for grofit function

control <- grofit.2.control()
...
pdf("Ecoli_20161014_OD_grofit.pdf",width=5, height=4)
par(mgp=c(1.7,.5,0),mai=c(.75,.75,.75,.1))
odfit <- gcFit.2(tmp$time, tmp$data,control)
dev.off()
## for presenteation in bergen 201611
plotdev("Ecoli_20161014_OD_grofit_A8",type="png",width=6, height=4)
par(mgp=c(1.7,.5,0),mai=c(.75,.75,.3,.1))
tmp <- gcFit.2(tmp$time[1,,drop=FALSE], tmp$data["pUC18mV_Inj:5001",,drop=FALSE],control)
dev.off()

lag <- odfit$gcTable[,"lambda.model"]
mu <- odfit$gcTable[,"mu.model"]
id <- odfit$gcTable[,"AddId"]
...
plotdev("Ecoli_20161014_OD_growthrates",type="png",width=6, height=4)
par(mgp=c(2.8,.5,0),mai=c(1,.75,.1,.1))
plot(mu*60~id,las=2,xlab=NA,ylab=expression("growth rate"~mu~", "~h^-1))
dev.off()

## cellGrowth
library(cellGrowth)
bw <- bandwidthCV.2(data,wells=unlist(groups),nWell=10,bandwidths=seq(1,600, length.out=30),scaleY=identity)
fit <- fitCellGrowth(data$Time,data$OD$data[,1],locfit.h=bw[[1]])
plot(fit)
abline(v=data$Time[attributes(fit)$pointOfMaxGrowthRate])
abline(v=data$Time[attributes(fit)$pointOfMax])

fits <- fitCellGrowths.2(data,did="mVenus",locfit.h=bw[[1]])
plot(fits[[1]])

## TODO: interpolate to OD
cut <- cutData(data,c(0,1250))
od.data <- interpolatePlateData(cut,"OD")

par(mfcol=c(1,3))
viewGroups(data,sgroups[2],groups,dids=c("OD","mV/OD"),lwd.orig=0,lty.orig=1,ylims=list("mV/OD"=c(-100,1e4)),show.ci95=TRUE,show.mean=TRUE,g2.legend=TRUE,emphasize.mean=TRUE,xlim=c(0,1500),embed=TRUE) #,log="y")
viewGroups(od.data,sgroups[2],groups,dids=c("mV/OD"),lwd.orig=0,lty.orig=1,ylims=list("mV/OD"=c(-100,1e4)),show.ci95=TRUE,show.mean=TRUE,g2.legend=TRUE,emphasize.mean=TRUE,xlim=c(0,.15),embed=TRUE) #,log="y")
par(mai=c(1,.75,.1,.1))
plot(mu*24 ~ id,las=2,xlab=NA,ylab=expression(mu~", "~d^-1))
