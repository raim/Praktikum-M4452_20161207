...
## BMG, Daten aus Platereader Axmann
files1 <- file.path(dpath,c("BMG/20161206_20161201 Praktikum - pRAJ11  1_Absorbance.CSV","BMG/20161206_20161201 Praktikum - pRAJ11  1_Fluorescence.CSV"))
...
## Synergy, Daten aus Platereader Kollmann
files2 <- c("Synergy/161207_Praktikum_realtime_GFP.csv")
...
viewGroups(raw1, groups=groups2, groups2=groups3,verb=F)

## Synergy, Daten aus Platereader Kollmann
viewGroups(raw2,groups=groups, groups2=groups2,verb=F)
viewGroups(raw2,groups=groups, groups2=groups3,verb=F)
viewGroups(raw2, groups=groups2, groups2=groups3,verb=F)

                                        # Prepare Data (blanks, cuts, etc. )
# Weiterarbeiten mit den Sympy Daten

data1 <- correctBlanks(raw1,plate)
data2 <- correctBlanks(raw2,plate)
...
##
od.data1 <- interpolatePlateData(data1, "OD")
od.data2 <- interpolatePlateData(data2, "OD")

## BMG, Daten aus Platereader Axmann
viewGroups(od.data1,groups=groups, groups2=groups2,verb=F)
viewGroups(od.data1,groups=groups, groups2=groups3,verb=F)
viewGroups(od.data1, groups=groups2, groups2=groups3,verb=F)

## Synergy, Daten aus Platereader Kollmann
viewGroups(od.data2,groups=groups, groups2=groups2,verb=F)
viewGroups(od.data2,groups=groups, groups2=groups3,verb=F)
viewGroups(od.data2, groups=groups2, groups2=groups3,verb=F)
